...
docs/
  adr/                 8 architectural decision records (ADR 0001–0008)
...
- **Session testing (E2E)** — `.session` script files drive the REAL telnet session handler in-process (no socket): login/account flow, command dispatch, prompt/dialogue routing, and the renderer — the wire-level paths `.test.luau` can't reach (a green Luau test while a command crashes on spawn, or gets swallowed as dialogue input). A file is alternating `> <input>` lines, `tick: <n>` advances, and `expect:`/`expect-not:` assertions (substring, or `/regex/`) matched against the plain-text a player reads (`markup::to_plain`). Deterministic without sleeps — the harness freezes the engine's wall-clock heartbeat and autosave, so time moves only where the script says `tick:`, and an `ApiRequest` fence guarantees each input's or tick's output is drained before the next. That covers `on_tick`, `after()` timers, and game-clock rollovers (`on_hour`/`on_dawn`/`on_dusk`). Run from the CLI (`hearth session-test <file>... [--config PATH] [--db PATH]`, building an engine in-process — no server needed) or in CI (the `tests/session_test.rs` glob runs `tests/fixtures/*.session` plus any `.session` files under the game's `game_dir`). `src/session_test.rs`.
...
- **REST API** — POST /api with 62 actions (list/examine, create room/object/exit, clone object, set title/desc/attr/tags/aliases, set/clear lock, update exit, force command (admin), script get/set/clear + lib get/set/remove, world_check, ink compile/save/load, maps + terrain, eval, import/export)
...
See `docs/adr/` (8 ADRs). See `CONTEXT.md` for domain glossary.

## Testing

439 tests across: softcode (127, incl. derive_hooks + grid_move), engine (118),
