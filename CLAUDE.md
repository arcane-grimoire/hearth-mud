...
- **Terrain hooks (terrain → archetype)** — a `[terrain.X]` palette entry may set `archetype = "area/key"`; every square of that terrain instantiates with `archetype_ref` pointing at it, so hooks defined on the archetype (`can_enter`/`can_traverse`/`on_enter`/`on_leave`/`on_tick`/`on_look`, or any custom `on_*` fired via `trigger()` like `on_combat`) apply to all squares of that terrain via normal archetype resolution — "lava burns", "deep water blocks" defined once. A single cell can point at its own archetype via a `[cells."x,y"]` override's `archetype` (a boss vent among ordinary lava), and any square can be reparented live with `set_archetype` since it's just a room. No new dispatch layer; reuses the archetype chain (`instantiate` resolves the key to a dbref; `get_map_template` surfaces it)
